...
dir.pres <- "OM 2 Small R0" #Change to whichever model you're working in
dir.rep <- "Reports"
...
#Env-start
  ...
  AR.index <- rnorm(N_envobs, mean = -sigG/2, sd = sqrt(sigG))
  for(t in 2:N_envobs) {
    AR.index[t] <- G.AR*AR.index[t-1] + AR.index[t]*sqrt(1-(G.AR^2))
  }

  SS3.index <- cbind(env.index$year, 1, AR.index)
...
#  last.MGparm <- rev(grep("MGparm", par.lines))[1]
#  temp <- as.numeric(unlist(strsplit(par.lines[last.MGparm], split = "[^[:digit:]]")))
#  last.par <- temp[!is.na(temp)]

#  for (x in 1:length(par2change)) {
#    beta.pars <- c(paste("# MGpar[", last.par + x, "]:", sep = ""), 1.00000)
#    par.lines <- c(par.lines[1:(last.MGparm+1)], beta.pars, par.lines[(last.MGparm+2):length(par.lines)])
#    last.MGparm <- last.MGparm+2
#  }
...
for (i in 1:Nboot) {
  ...
  sim.SSB <- EM2.reps[[i]]$recruit$spawn_bio[1:Nyears]
...
for (i in 1:Nboot) {
  ...
  sim.Rec <- EM2.reps[[i]]$recruit$exp_recr[1:length(true.Rec)]
...
for(parm in 1:length(par2change)) {
  ...
  for (i in 1:Nboot) {
    true.par <- eval(parse(text=paste("true.reps[[i]]$MGparmAdj$",par2change,sep = "")[parm]))
    sim.par <- eval(parse(text=paste("EM2.reps[[i]]$MGparmAdj$",par2change,sep = "")[parm]))
    par.err[i,] <- (sim.par-true.par) / true.par
  }
  for (j in 1:length(true.par)) {
  ...
  plot(0, xlim=c(modStartYr, modEndYr), ylim=c(-0.01,0.01),xaxs="i",ylab="Error", xlab = "Year")
  ...
  abline(h=0, col="red")
  mtext(paste("Standardised Errors for ", par2change[parm], sep = ""), side = 3)
}
par(mfrow = c(1,1))
...
for(parm in 1:length(envparnames)) {
  trueval <- true.reps[[3]]$parameters[which(true.reps[[1]]$parameters$Label==envparnames[parm]),]$Value
...
sumtrueyield <- 0
for(i in 1:Nboot) {
  true.yield <- sum(true.reps[[i]]$derived_quant[grep("ForeCatchret", true.reps[[i]]$derived_quant[,1]),2])
  yield.1 <- sum(reps.ctl1[[i]]$derived_quant[grep("ForeCatchret", reps.ctl1[[i]]$derived_quant[,1]),2])
  yield.2 <- sum(reps.ctl2[[i]]$derived_quant[grep("ForeCatchret", reps.ctl2[[i]]$derived_quant[,1]),2])
