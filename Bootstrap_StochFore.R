...
dir.pres <- "OM 4 Proj TVG" #Change to whichever model you're working in
dir.rep <- "Reports"
...
foreach(i=1:Nboot) %dopar% {
  ...
  EnvYrs <- seq(from = styr, to = (endyr + foreyears), by = 1)
...
for(i in 1:Nboot) {
  ...
  true.MSY[i] <- subset(true.reps[[i]]$derived_quants, LABEL == "TotYield_MSY")$Value
  sim1.MSY[i] <- subset(EM1.reps[[i]]$derived_quants, LABEL == "TotYield_MSY")$Value
  sim2.MSY[i] <- subset(EM2.reps[[i]]$derived_quants, LABEL == "TotYield_MSY")$Value

  true.finSSB[i] <- true.reps[[i]]$recruit$spawn_bio[Nyears]
  sim1.finSSB[i] <- EM1.reps[[i]]$recruit$spawn_bio[Nyears]
  sim2.finSSB[i] <- EM2.reps[[i]]$recruit$spawn_bio[Nyears]

  true.initSSB[i] <- true.reps[[i]]$derived_quants$Value[1] 
  sim1.initSSB[i] <- EM1.reps[[i]]$derived_quants$Value[1]  
  sim2.initSSB[i] <- EM2.reps[[i]]$derived_quants$Value[1]
}
...
median(true.finSSB)/1e9
median(sim1.finSSB)/1e9
median(sim2.finSSB)/1e9

sd(sim1.finSSB)/1e9
sd(sim2.finSSB)/1e9

median(true.initSSB)/1e9
median(sim1.initSSB)/1e9
median(sim2.initSSB)/1e9
