...
foreach(i=1:Nboot) %dopar% {
  ...
  new_N_envobs <- Yrs2keep+foreyears

  SS3.index <- dat.file$envdat[(N_envobs - new_N_envobs + 1):N_envobs,]

  dat.file$envdat <- data.frame(SS3.index)
  dat.file$N_environ_obs <- new_N_envobs
  ...
  system(exe.cmd, show.output.on.console = FALSE)
   ...
   ##########################################
    ...
    file.copy(from = "Report.sso", to = file.path(dir.check,dir.pres,dir.rep,rep.name), overwrite = TRUE)

}

for(i in 1:100) {
setwd(dir.base)
dir.boot <- paste0("Bootstrap-", i)
setwd(dir.boot)
if(dir.exists("EM-4")) {
print(i)
unlink("EM-4", recursive = TRUE)
}
else{next}
}